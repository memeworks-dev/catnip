 ...
 * is stubbed until real providers are keyed). Produces a self-contained SVG — no
 * network — themed with the toy's brand colours, the user's caption, brand
 * attribution, and the "Made with Catnip" watermark, so the result screen
 * previews the real share card (§4, §9) without a model call.
 *
 * Used by the `local` generation provider (lib/generation/providers/local) as a
 * dev-only fallback when no real provider key is configured.
 */
...
/** Raw SVG markup for the placeholder meme. */
export function buildPlaceholderSvg(opts: PlaceholderMemeOptions): string {
...
/** Placeholder meme as a data URI (for direct use in <img src>). */
export function buildPlaceholderMeme(opts: PlaceholderMemeOptions): string {
  return `data:image/svg+xml;utf8,${encodeURIComponent(buildPlaceholderSvg(opts))}`;
}
